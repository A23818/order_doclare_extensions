...
function addTaskBar(domain) {
    ...
    chrome.storage.sync.get("translateWarning", function(result) {
        html = "<div class='doclare-toggle-container'>";
        if (result.translateWarning || typeof result.translateWarning === 'undefined') {
            html += "<a href='javascript:;' id='doclare-toggle-addon'>Thu gọn</a>";
        } else {
            html += "<a href='javascript:;' id='doclare-toggle-addon'>Mở rộng</a>";
            $(".doclare-alert").addClass("collapsed");
            $(".doclare-alert").hide();
        }
        html += "</div>";
        $(html).appendTo(document.body);
    ...
    setTimeout(function() {
        ...
        $("#doclare-toggle-addon").click(function() {
            if ($(".doclare-alert").hasClass('collapsed')) {
                $(this).text("Thu gọn");
                $(".doclare-alert").removeClass('collapsed');
                ...
                if (!$.isEmptyObject(shop)) {
                    if (Array.isArray(product) && product.length > 0) {
                        ...
                        $.each(product, function() {
                            ...
                            this.note = $("#doclare-textarea").val();
                        ...
                        if (parseInt(product.quantity) <= parseInt(product.stock)) {
                            product.note = $("#doclare-textarea").val();
