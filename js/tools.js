...
var tools = function() {
    var self = this;
    self.baseUrl = "https://order.doclare.com/";
    var addUrl = "ext/add";
        ...
        self.sendAjax(addUrl, "POST", cart,
            function(resp) {
                if (resp && resp.success) {
                    ...
                    $("#myDLRModal-order").DLRModal("toggle");
                    // var totalAmount = 0;
                    // $.each(cart.items, function (index, value) {
                    //     totalAmount += value.totalAmountNDT;
                    // });
                    // totalAmount = Math.round(totalAmount * 100) / 100;
                    $(".ls-deposit p").text("Sản phẩm được thêm thành công vào giỏ hàng của bạn");
                    $("#doclare-textarea").val("");
                }
            }
        );
    };
