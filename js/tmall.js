var tmall = function() {
    ...
    self.getProduct = function() {
        ...
        prod.updateView = function() {
            ...
            $('.doclare-box-info #stock').text(stock);
        };
    ...
    self.renderView = function(product) {
        ...
        // }
        $('.doclare-box-info #stock').text(product.stock);
        if (product.itemPrice) {
            $('.doclare-box-info #sell_price').text(product.itemPrice + "đ");
        } else {
            $('.doclare-box-info #sell_price').text(tool.convertToVND(product.itemPriceNDT) + "đ");
        }
        if ($("#J_Sold-out-recommend").length) {
            $(".doclare-warning").text("Sản phẩm này đã hết hàng");
        }
    };
