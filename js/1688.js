var cn1688 = function() {
        ...
        prod.updateView = function() {
            ...
            if (highlight && highlight.length) {
                ...
                if (price) {
                    ...
                    $('.doclare-box-info #sell_price').text(priceVnd);
                }
            }
        };
    ...
    self.renderView = function(product) {
        ...
        $('.doclare-box-info #stock').text(totalAmount);
        //Display price
        if (product.itemPrice && Array.isArray(product.itemPrice) && product.itemPrice.length > product.wholesales.length) {
            ...
            $('.doclare-box-info #sell_price').text(displayPrice);
            ...
            $.each(product.wholesales, function(index, range) {
                priceTable += "<tr><td>Mua ≥ " + range.begin + " sản phẩm</td>" +
                    "<td class='text-doclare'>" + tool.convertToVND(range.price) + "đ </td></tr>";
            });
            $(".doclare-box-info .doclare-1688-price").html(priceTable);
            $(".doclare-box-info .doclare-advanced-info").removeClass('hidden');
            $('.doclare-box-info #sell_price').text(tool.convertToVND(product.itemPriceNDT) + "đ");
        } else {
            $('.doclare-box-info #sell_price').text(tool.convertToVND(product.itemPriceNDT) + "đ");
        }
        // Display quantity condition
        if (product.requireMin > 1) {
            $(".doclare-warning").text("Cửa hàng yêu cầu mua tối thiểu " + product.requireMin + " sản phẩm");
            $(".doclare-warning").removeClass('hidden');
        }
    };
